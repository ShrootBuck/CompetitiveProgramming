...
bool impossible_path(int row, int col) {
    ...
    if (is_blocked(row - 1, col) && is_blocked(row + 1, col) && !is_blocked(row, col - 1) &&
        !is_blocked(row, col + 1)) {
        // This path is a horizontal wall, doomed to fail
        return true;
    }

    if (is_blocked(row, col - 1) && is_blocked(row, col + 1) && !is_blocked(row - 1, col) &&
        !is_blocked(row + 1, col)) {
        // Same as above, but this is a vertical wall
        return true;
    }

    // Up-Right pocket
    if (is_blocked(row - 1, col) && is_blocked(row, col + 1) && !is_blocked(row - 1, col + 1)) {
        return true;
    }
    // Up-Left pocket
    if (is_blocked(row - 1, col) && is_blocked(row, col - 1) && !is_blocked(row - 1, col - 1)) {
